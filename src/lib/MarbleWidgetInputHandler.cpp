        ...
        if ( activeRegion.contains( event->pos() ) || d->m_selectionRubber.isVisible() ) {
            ...
            if ( d->m_leftPressed && !d->m_selectionRubber.isVisible() ) {
                ...
                if ( abs( deltax ) > d->m_dragThreshold
                    ...
                    const qreal lon = quat.sphericalLon();
                    const qreal lat = quat.sphericalLat();
                    MarbleWidgetInputHandler::d->m_widget->centerOn( RAD2DEG * ( qreal )( d->m_leftPressedLon + lon ),
                                                                     RAD2DEG * ( qreal )( d->m_leftPressedLat + lat ) );
                }
            }
